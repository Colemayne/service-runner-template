...
const (
	defaultServiceName = "service-runner"
	defaultDescription = "A simple service runner"
)
...
func runServiceInstall(s service.Service, c *cli.Context) error {
	if user := c.String("user"); user == "" && os.Getuid() == 0 {
		logrus.Fatal("Please specify user that will run service-runner service")
	}
...
func getServiceArguments(c *cli.Context) (arguments []string) {
	...
	// Removing config until there is a need for it.
	/*
...
// RunServiceControl allows inputting commands to services.
func RunServiceControl(c *cli.Context) error {
	svcConfig := createServiceConfig(c)

	s, err := serviceHelpers.New(&NullService{}, svcConfig)
	if err != nil {
		logrus.Fatal(err)
	}
